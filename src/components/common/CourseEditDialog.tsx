...
interface CourseEditDialogProps {
  isOpen: boolean;
  course: Course | null;
  onSave: (courseId: number, courseData: CourseUpdateData) => Promise<void>;
  onCancel: () => void;
}
      ...
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="relative inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
          <form onSubmit={handleSubmit}>
            ...
            <div className="sm:flex sm:items-start mb-6">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <p className="text-sm text-gray-500">
                  Update course details, role, and account status.
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-3">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-700">Course Published</span>
                  </label>
                </div>
            ...
            {/* Current Course Info Display */}
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <h4 className="text-sm font-medium text-gray-900 mb-2">Current Course Information</h4>
              <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                <div>ID: {course.id}</div>
                <div>Created: {new Date(course.createdAt).toLocaleDateString()}</div>
                <div>Last Updated: {course.updatedAt ? new Date(course.updatedAt).toLocaleDateString() : 'Never'}</div>
              </div>
            </div>
