...
interface LessonAddDialogProps {
  isOpen: boolean;
  onSave: (lessonData: LessonCreateData) => Promise<void>;
  onCancel: () => void;
}
...
const LessonAddDialog: React.FC<LessonAddDialogProps> = ({
  isOpen,
  onSave,
  onCancel
}) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Clear error when lesson starts typing
    if (errors[name]) {
      setErrors(prev => ({
        ...prev,
        [name]: ''
 ...
 const validateForm = () => {
  ...
  // lessonOrder and duration are optional; default values will be used if left as is
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 text-black ${
                    errors.lessonOrder ? 'border-red-300 focus:ring-red-400' : 'border-gray-300'
                  }`}
                  placeholder="Enter lesson order"
                /> 
                {errors.lessonOrder && (
                  <p className="mt-1 text-xs text-red-600">{errors.lessonOrder}</p>
                )}
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center space-x-3">
                <label className="flex items-center">
                  ...
                  />
                  <span className="ml-2 text-sm text-gray-700">Lesson Published</span>
                </label>
              </div>
              ...
              <div className="flex items-center space-x-3">
                <label className="flex items-center">
                  <input    
                    ...
                    className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                  />
                  <span className="ml-2 text-sm text-gray-700">Lesson Free</span>
                </label>
              </div>
              <div className={`mt-1 px-2 py-1 rounded text-xs font-medium inline-block ${
                formData.isFree 
                 ...
                 Status: {formData.isFree ? 'Free' : 'Paid'}
              </div>
            </div>
...
export default LessonAddDialog;
